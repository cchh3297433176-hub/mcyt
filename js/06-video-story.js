        ...
        function buildSystemPrompt() {
            ...
            const historySummary = activeStoryHistory.slice(-8).map(h =>
                h.truncated
                    ...
                    : `[第${h.day}天 ${getTimeSlotName(h.time)}] ${h.text.slice(0, 100)}...`
            ).join('\n');
            const memorySummaryText = (G.memorySummaries || []).map(m =>
                `[记忆总结 · 截至第${m.day}天] ${m.text}`
            ).join('\n');
            ...
            for (const [id, npc] of Object.entries(G.npcs)) {
                ...
                const recent = hist.slice(-5).map(m => `${m.from === 'npc' ? npc.name : '玩家'}: ${m.text.slice(0, 40)}...`)
                    .join('\n');
        ...
        async function generateStory(tag, userPrompt, useSearch = false, replaceBlock = null, replaceHistoryId = null) {
            ...
            try {
                ...
                // 重说时必须把“正在被替换的那一条”从 AI 上下文中排除，
                // 否则模型很容易把旧剧情当成上一轮上下文继续复述。
                ...
                for (const h of recentHistory) {
                    const snippet = h.truncated ? '（此前一条内容生成不完整，已忽略）' : h.text.slice(0, 500) + '...';
                ...
                if (!replaceBlock) {
                    ...
                    // 重说：直接替换原剧情块和对应历史记录，而不是删除旧块后再新增。
                    // 这样即使滚动位置、DOM 引用或其他 UI 状态发生变化，也不会留下旧剧情显示。
        ...
        async function regenerateStoryBlock(block, tag, userPrompt, useSearch) {
            ...
            // 不再提前删除旧剧情。只有 AI 成功返回后才替换，避免出现“生成成功但页面仍显示旧内容”的状态。
        ...
        function buildEditEntryHTML(entry, kind) {
            ...
            const preview = escapeHtml((entry.text || '').slice(0, 50)) + ((entry.text || '').length > 50 ? '...' : '');
            return `
            <div class="edit-entry" data-id="${entry._id}" data-kind="${kind}" style="margin-bottom:8px;border:1px solid rgba(30,60,30,.10);border-radius:10px;overflow:hidden;">
                <div class="edit-entry-header" style="cursor:pointer;display:flex;justify-content:space-between;align-items:center;padding:8px 10px;background:#f5faf5;">
                    <div style="font-size:12px;color:#666;flex:1;min-width:0;">
                        ...
                        <div class="edit-entry-preview" style="color:#999;margin-top:2px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">${preview}</div>
                    </div>
                ...
                <div class="edit-entry-body" style="display:none;padding:8px 10px 10px;background:#fff;">
                    <textarea class="edit-textarea" style="width:100%;min-height:140px;padding:8px;border-radius:8px;border:2px solid rgba(30,60,30,.10);background:#f5faf5;color:var(--text);font-size:13px;font-family:inherit;resize:vertical;">${escapeHtml(entry.text || '')}</textarea>
        ...
        function buildChatEditEntryHTML(npcName, msg) {
            ...
            const preview = escapeHtml((msg.text || '').slice(0, 50)) + ((msg.text || '').length > 50 ? '...' : '');
            return `
            <div class="edit-entry" data-id="${msg._id}" style="margin-bottom:8px;border:1px solid rgba(30,60,30,.10);border-radius:10px;overflow:hidden;">
                ...
                <div class="edit-entry-body" style="display:none;padding:8px 10px 10px;background:#fff;">
                    <textarea class="edit-textarea" style="width:100%;min-height:100px;padding:8px;border-radius:8px;border:2px solid rgba(30,60,30,.10);background:#f5faf5;color:var(--text);font-size:13px;font-family:inherit;resize:vertical;">${escapeHtml(msg.text || '')}</textarea>
                    <div class="btn-row" style="margin-top:8px;">
                        <button class="btn-secondary edit-cancel-btn">取消</button>
                        <button class="btn-primary edit-save-btn">💾 保存修改</button>
                    </div>
        ...
        async function generateMemorySummary(keepRecentCount, opts = {}) {
            ...
            const priorSummaries = (G.memorySummaries || []).map(m => m.text).join('\n');
            let summaryText;
            if (opts.manualText) {
                summaryText = opts.manualText.trim();
            } else {
                const combinedText = toSummarize.map(h =>
                    `[第${h.day}天 ${getTimeSlotName(h.time)}] ${h.text}`
                ).join('\n\n');
            ...
            <div style="margin-top:16px;border-top:1px solid rgba(30,60,30,.10);padding-top:10px;">
                ...
                <div style="max-height:26vh;overflow-y:auto;font-size:12px;color:#666;">
                    ${summaries.length ? [...summaries].reverse().map(sm =>
                        `<div style="padding:6px 0;border-bottom:1px dashed rgba(30,60,30,.10);"><b>截至第${sm.day}天</b>（归档${sm.coveredCount}轮${sm.manual ? '·手动' : ''}）：${escapeHtml(sm.text.slice(0, 80))}${sm.text.length > 80 ? '...' : ''}</div>`
                    ).join('') : '暂无'}
                </div>
