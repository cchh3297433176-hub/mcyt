        ...
        function renderChatList(container) {
            ...
            for (const [id, npc] of Object.entries(G.npcs)) {
                ...
                const preview = lastMsg ? lastMsg.text : '开始对话吧';
                const time = lastMsg ? (lastMsg.time ? lastMsg.time.slice(5, 16) : '') : '';
        ...
        function renderChatWindow(container) {
            ...
            for (const msg of chatHist) {
                const cls = msg.from === 'player' ? 'self' : (msg.from === 'system' ? 'other' : 'other');
        ...
        async function getNPCReply(npcId, playerText) {
            ...
            let npcSpecific = '';
            if (npcId === 'twixxel') npcSpecific = '你性格温和，胆子不大但敢于住进旧屋，说话自然。';
            else if (npcId === 'xqree') npcSpecific = '你外表温和，实际敢爱敢恨，对喜欢的人温柔，声音好听。';
            else if (npcId === 'groxmc') npcSpecific = '你自我中心，说话随意，常用口头禅"hey yo chill"、"Alright bet"。';
            else if (npcId === 'dream') npcSpecific = '你神秘、自信、掌控欲强，说话简洁有力，带点神秘感。';
            else if (npcId === 'thatmob') npcSpecific = '你随和健谈，幽默带点傲娇，20岁加拿大/法国人。';
            else if (npcId === 'whispy') npcSpecific =
            '你充满活力，好感度40以下较冷淡，40以上话唠粘人，被叫"小南瓜"。有自己的脾气，会生气，真生气了会一声不吭。';
            const sysPrompt = `
            ...
            3. 可以自由谈论Minecraft、游戏、直播、红石等话题。
            4. 请结合最近3条聊天记录生成回复，保证逻辑连贯。
            【⚠️ 绝对禁令 -- 禁止复读】
            你绝对禁止复读或重复玩家刚刚说过的话。
            你的回复必须是对玩家话题的延伸、新观点或补充信息。
            严禁以"你刚才说...."或引用玩家原句开头。
            总之：承接话题，延伸新内容，绝不复读。
            【聊天规范】
            严禁在回复中使用括号或任何动作/语气描述。只输出纯文本对话内容。
            模拟手机聊天的自然语气。
            【语气与风格】
            根据好感度调整亲密度：${tone}，回复长度${lengthHint}。
            主体用中文，可穿插英文语气词（hmm, oh, hey, well），但不要过度。
            【额外人设细节】${npcSpecific}
            最近聊天记录（供参考，保证连贯）：${recentHistory || '（无最近记录）'}
            玩家消息：${playerText}
            只输出你的回复内容。
            `;
            try {
                const reply = await callAI([{ role: 'system', content: sysPrompt }, { role: 'user',
                        content: playerText }], { maxTokens: 10000, temperature: 0.8 });
                const now = new Date();
                const timeStr = now.toLocaleTimeString();
                if (!G.chatHistory[npcId]) G.chatHistory[npcId] = [];
                ...
                G._lastRegenerate = () => regenerateNPCReply(npcId, playerText);
            } catch (e) {
                console.error('NPC回复失败', e);
                const fallback = ['Hmm.', 'Alright.', 'I see.', 'Yeah.', 'Nah.', 'Cool.', 'Sure.', 'Oh.'][rand(0, 7)];
                if (!G.chatHistory[npcId]) G.chatHistory[npcId] = [];
                const now = new Date();
        ...
        function regenerateNPCReply(npcId, playerText) {
            if (G.isGenerating) { showToast('⏳ 正在生成中，请稍候'); return; }
            const hist = G.chatHistory[npcId];
            if (hist && hist.length && hist[hist.length - 1].from === 'npc') hist.pop();
            if (G.currentChatNpc === npcId) renderSocialPanel();
            getNPCReply(npcId, playerText);
        }
        // ============================================================
        // 回忆录
        // ============================================================
