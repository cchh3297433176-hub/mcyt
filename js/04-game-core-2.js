...
// 彻底解决 Android WebView 长按与短按冲突（稳定进入聊天）
function bindLongPressEvent(el, onLongPress, onClick) {
    ...
    el.addEventListener('touchmove', e => {
        ...
        if (Math.abs(e.touches[0].clientX - startX) > MOVE_LIMIT ||
            Math.abs(e.touches[0].clientY - startY) > MOVE_LIMIT) {
            isMoved = true;
    ...
    el.addEventListener('touchend', e => {
        if (timer) { clearTimeout(timer); timer = null; }
        if (isPressing && !isMoved && !isLongTriggered && onClick) {
            e.preventDefault();
            touchConsumed = true;
