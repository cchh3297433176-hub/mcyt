...
function bindChatListEvents(container) {
    ...
    document.getElementById('tabGroupBtn')?.addEventListener('click', () => { 
...
function renderSingleChatWindow(container) {
    ...
    document.getElementById('singleChatBackBtn')?.addEventListener('click', () => closeChat());
    ...
    container.querySelectorAll('.chat-npc-avatar-btn').forEach(btn => {
        btn.addEventListener('click', () => openNpcProfileCardModal(npcId));
    });

    document.getElementById('singleChatHeaderProfileBtn')?.addEventListener('click', () => {
...
function renderGroupChatWindow(container) {
    ...
    <div style="background:#f2f4f2;border-radius:14px;display:flex;flex-direction:column;height:100%;min-height:500px;box-shadow:0 4px 16px rgba(0,0,0,0.08);overflow:hidden;">
        ...
        <div style="padding:8px 10px;background:#fff;border-top:1px solid #e5ebe5;display:flex;gap:8px;align-items:center;flex-shrink:0;">
            <button id="groupActionInsertBtn" title="共创视频/开播" style="border:1px solid #ccc;background:#f8f9f8;color:#555;width:36px;height:36px;border-radius:50%;font-size:20px;cursor:pointer;display:flex;align-items:center;justify-content:center;flex-shrink:0;">➕</button>
            <button id="groupToggleStickerBtn" title="发送表情包" style="border:1px solid #ccc;background:${_stickerDrawerOpen ? '#eaf5ea' : '#f8f9f8'};color:#555;width:36px;height:36px;border-radius:50%;font-size:17px;cursor:pointer;display:flex;align-items:center;justify-content:center;flex-shrink:0;">😊</button>
            ...
            <button id="groupSendBtn" style="border:none;background:var(--primary);color:#fff;padding:8px 16px;border-radius:18px;font-size:13px;font-weight:700;cursor:pointer;flex-shrink:0;">发送</button>
        </div>
    </div>
    `;
    container.innerHTML = html;
    ...
    document.getElementById('groupChatBackBtn')?.addEventListener('click', () => closeGroupChat());
...
function openAddChatTargetModal() {
    ...
    document.getElementById('btnNewCustomNPC')?.addEventListener('click', () => {
        closeModal();
...
function openCreateCustomNpcModal() {
    ...
    document.getElementById('btnConfirmCreateNpc')?.addEventListener('click', () => {
...
function openCreateGroupModal() {
    ...
    document.getElementById('btnConfirmCreateGroup')?.addEventListener('click', () => {
...
function openGroupSettingsModal(gid) {
    ...
    openModal(`
        ...
        <div style="font-size:13px;font-weight:700;margin-bottom:6px;">👥 成员列表</div>
...
function buildMomentsHTML() {
    ...
    return `
    <div style="padding:10px 14px;background:#f8fbf8;border-bottom:1px solid #eef3ee;display:flex;justify-content:space-between;align-items:center;gap:6px;flex-shrink:0;">
...
function bindMomentsEvents(container) {
    ...
    container.querySelectorAll('.moment-ai-cmt-btn').forEach(btn => {
        btn.onclick = async () => {
            await triggerAiCommentForMoment(parseInt(btn.dataset.id));
        };
    });
