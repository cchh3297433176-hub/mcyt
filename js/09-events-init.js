...
// ============================================================
// ============================================================
// 修复 App 从后台恢复时，WebView 滚动区域触摸失效/卡死的问题
// ============================================================
let _reflowTimer = null;
function forceReflowFix() {
    if (_reflowTimer) clearTimeout(_reflowTimer);
    
    // 延迟一点点，确保 WebView 视口已经完全恢复
    _reflowTimer = setTimeout(() => {
        ...
        // 2. 温和地修复滚动区域（绝对不能用 display: none）
        const scrollAreas = document.querySelectorAll('.page.active, #storyArea, .tab-content, .modal-box, .scroll-y');
        scrollAreas.forEach(el => {
            ...
            // 温和触发重绘：使用透明度或者微小的 transform 变化
    ...
    }, 150); // 增加延迟，给 WebView 留出恢复渲染的时间
}

// 仅使用 visibilitychange 足矣，去掉 focus 和 pageshow，防止事件打架和重复执行
document.addEventListener('visibilitychange', () => {
    if (document.visibilityState === 'visible') {
        forceReflowFix();
    } else {
        // 切到后台时，主动让页面失焦，防止输入法键盘残留导致的视口高度 bug 卡死滚动
