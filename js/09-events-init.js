...
// 事件绑定与全新微信简约风三页滑动公告系统（v1.620 完整收录李敏原话与作者心声 · 修复缺失函数）
// ============================================================
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (typeof loadSavedModels === 'function') loadSavedModels();
    if (typeof loadMemorySummarySettings === 'function') loadMemorySummarySettings();
...
function openEditPlayerProfileModal() {
    const p = (window.G && window.G.player) ? window.G.player : {};
    ...
    document.getElementById('savePlayerProfileBtn')?.addEventListener('click', () => {
        ...
        if (nameChanged) {
            ...
            if (typeof addMemoir === 'function') addMemoir('更名启事', `频道名由「${oldName}」更改为「${newName}」`);
            if (typeof appendStory === 'function') appendStory(`📢 你的频道正式更名为「${newName}」，粉丝与好友们都在为你庆祝新起点！`, '📢 频道更名');
        }
...
function updateWebSearchToggleUI() {
    ...
    if (!btn) return;
    btn.classList.toggle('gold', !!(window.G && window.G.search && window.G.search.enabled));
}
...
function openActionModal(action) {
    ...
    const hasSearchConfigured = !!(window.G && window.G.search && (window.G.search.enabled || window.G.search.apiKey || (window.G.search.keys && Object.values(window.G.search.keys).some(k => !!k))));
    ...
    <div class="form-group" style="display:flex;align-items:center;gap:8px;margin-top:2px;">
        <label style="font-size:13px;margin-bottom:0;display:flex;align-items:center;gap:6px;cursor:pointer;">
            <input type="checkbox" id="modalUseSearch" ${window.G.search.enabled ? 'checked' : ''} style="width:16px;height:16px;accent-color:var(--primary);">
    ...
    document.getElementById('modalConfirmAction')?.addEventListener('click', async () => {
        ...
        } else if (typeof performAction === 'function') {
            await performAction(action, detail, useSearch);
        }
    });
}
