...
(function() {
            ...
            mask.innerHTML = `
                <div class="wechat-clean-modal-card" style="max-width:350px;width:92%;padding:16px;position:relative;background:#ffffff;border-radius:12px;box-shadow:0 8px 30px rgba(0,0,0,0.14);box-sizing:border-box;max-height:90vh;display:flex;flex-direction:column;">
                    <div style="font-size:14.5px;font-weight:600;color:#181818;margin-bottom:10px;display:flex;justify-content:space-between;align-items:center;">
                        <span style="overflow:hidden;text-overflow:ellipsis;white-space:nowrap;padding-right:8px;">装扮中心 · ${escapeHtml(npc.name || 'NPC')}</span>
                        <button type="button" id="btnDecorModalX" style="border:none;background:#f2f2f2;border-radius:50%;width:22px;height:22px;color:#777;cursor:pointer;font-size:12px;flex-shrink:0;">✕</button>
                    </div>

                    <!-- 顶部主分区切换 -->
                    <div style="display:flex;background:#f2f2f2;border-radius:8px;padding:2px;margin-bottom:12px;flex-shrink:0;">
                    ...
                    <div style="flex:1;overflow-y:auto;display:flex;flex-direction:column;gap:12px;padding-right:2px;">
                            ...
                            <div style="display:flex;flex-direction:column;gap:12px;">
                                ...
                                <div style="background:#f8f9fa;border-radius:8px;padding:10px;border:0.5px solid #eee;">
                                    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px;gap:8px;">
                                        <div style="font-size:12px;font-weight:600;color:#333;flex-shrink:0;white-space:nowrap;">当前舞台方案：</div>
                                        <div style="display:flex;gap:4px;flex-shrink:0;">
                                            <button type="button" id="btnExportCurrentProfile" title="导出当前方案" style="border:1px solid #dcdcdc;background:#fff;border-radius:4px;padding:3px 7px;font-size:11px;color:#555;cursor:pointer;white-space:nowrap;">导出</button>
                                            <button type="button" id="btnImportProfileDirect" title="导入方案" style="border:1px solid #dcdcdc;background:#fff;border-radius:4px;padding:3px 7px;font-size:11px;color:#555;cursor:pointer;white-space:nowrap;">导入</button>
                                            <button type="button" id="btnAddNewStageProfile" title="新建配组" style="border:none;background:#07c160;color:#fff;border-radius:4px;padding:3px 8px;font-size:11px;cursor:pointer;font-weight:600;white-space:nowrap;">+ 新建</button>
                                        </div>
                                    </div>
                                    ...
                                    <div style="display:flex;justify-content:space-between;align-items:center;margin-top:8px;padding-top:6px;border-top:0.5px dashed #eee;gap:6px;">
                                        <div style="font-size:11px;color:#888;flex-shrink:0;white-space:nowrap;">重命名：</div>
                                        <div style="display:flex;gap:6px;flex:1;min-width:0;">
                                            <input type="text" id="inputStageProfileName" value="${escapeHtml(activeProf.name)}" class="wechat-clean-input" style="font-size:11px;padding:3px 6px;height:24px;flex:1;min-width:0;">
                                            <button type="button" id="btnRenameProfile" style="border:none;background:#07c160;color:#fff;border-radius:4px;padding:0 8px;font-size:10.5px;cursor:pointer;flex-shrink:0;white-space:nowrap;">改名</button>
                                        </div>
                                        ${videoStageData.profiles.length > 1 ? `
                                            <button type="button" id="btnDeleteCurrentProfile" style="border:none;background:none;color:#fa5151;font-size:11px;cursor:pointer;padding:0 4px;flex-shrink:0;white-space:nowrap;">删除</button>
                                        ` : ''}
                                    </div>
                                </div>
                                        ...
                                        ${allExprs.map(e => {
                                            ...
                                            return `
                                                <div class="sprite-slot-card" data-eid="${e.id}" data-label="${escapeHtml(e.label)}" style="display:flex;flex-direction:column;align-items:center;background:#fff;border:1px solid ${spriteUrl ? '#07c160' : '#e0e0e0'};border-radius:6px;padding:5px 1px;cursor:pointer;position:relative;box-sizing:border-box;min-width:0;">
                                                    ...
                                                    <span style="font-size:10px;color:#333;margin-top:4px;font-weight:500;text-align:center;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;width:100%;display:block;">${escapeHtml(e.label)}</span>
                                                </div>
                                            `;
                                        }).join('')}
                                    </div>
                        ...
                        ` : `
                            <!-- 基础头像框与气泡样式分区 -->
                            <div style="display:flex;background:#f2f2f2;border-radius:8px;padding:2px;margin-bottom:4px;flex-shrink:0;">
    ...
    // 🌟 导出单舞台方案（采用标准 Base64 DataURL 通道，穿透 WebView 限制直接落盘 Download 目录）
    function exportSingleStageProfile(profile, charName) {
        try {
            ...
            if (typeof showToast === 'function') showToast('舞台方案已保存至系统下载目录', 'success', 1200);
        } catch (e) {
            console.error('[ExportProfile] 导出失败:', e);
            if (typeof showToast === 'function') showToast('导出方案失败', 'error');
        }
    }
