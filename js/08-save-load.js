        ...
        // 自动存档：每次行动后静默写入 localStorage，换新版本 HTML 文件（同源同目录）时可一键继续
        ...
        function resumeAutoSave() {
            ...
            if (!saveData) { showToast('❌ 没有可继续的进度', 'error'); return; }
            try {
                applySaveData(saveData.data);
                dom.setupPage.classList.remove('active');
                dom.gamePage.classList.add('active');
                dom.storyArea.innerHTML = '';
                appendStory(`▶️ 已自动继续上次的游戏进度！\n第 ${G.day} 天，继续你的主播生涯吧！`, '▶️ 自动继续');
                updateUI();
                switchTab('story');
                renderAllPanels();
                showToast(`▶️ 已继续上次进度！第 ${G.day} 天`, 'success', 3000);
                checkAchievements();
                checkMilestones();
            } catch (e) {
                console.error('继续进度失败', e);
                showToast('❌ 继续进度失败，数据可能已损坏', 'error');
            }
        }
        ...
        function loadGameFromSlot(slotIndex) {
            ...
            try {
                ...
                dom.gamePage.classList.add('active');
                dom.storyArea.innerHTML = '';
                appendStory(`📂 已从存档位 ${slotIndex} 读档成功！\n继续你的主播生涯吧！`, '📂 读档成功');
        ...
        function showSaveSlotsModal(mode, exitAfter = false) {
            ...
            if (autoInfo) {
                ...
                <div class="save-slot-item" id="autoSaveLoadItem" style="border-color:var(--primary);">
                    <div class="slot-info">
                        <div class="slot-label">⏱️ 自动存档（往日回忆）</div>
        ...
        function confirmExitGame() {
            ...
            openModal(`
                ...
                <p style="font-size:13px;color:#666;line-height:1.6;">
                    ${canSave ? '是否保留当前游戏内容作为存档？保存后，下次进入 App 会自动为你展示「往日回忆 / 新记忆」选择页。' : '当前还没有可保存的游戏进度，确定要退出吗？'}
                </p>
                <div class="btn-row" style="margin-top:14px;flex-direction:column;gap:8px;">
                    ${canSave ? `<button class="btn-primary" id="exitSaveBtn" style="width:100%;">💾 保存后退出</button>` : ''}
                    <button class="btn-secondary" id="exitNoSaveBtn" style="width:100%;">🚫 不保存，直接退出</button>
